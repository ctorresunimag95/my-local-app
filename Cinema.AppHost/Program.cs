...
var management = builder.AddProject<Projects.Cinema_Management>("management")
    ...
    .WithEnvironment("omdb__apiUrl", builder.Configuration.GetValue<string>("omdb:apiUrl")!);
...
if (builder.Environment.IsDevelopment() && !builder.ExecutionContext.IsPublishMode)
{
    ...
    management.WithReference(serviceBus, connectionName: "serviceBus")
        .WithReference(cosmos, connectionName: "cosmosDb")
        .WaitFor(serviceBus)
        .WaitFor(cosmos);

    reservation.WithReference(serviceBus, "serviceBus")
        .WaitFor(serviceBus)
...
} else
{
    ...
    var cosmosDbConnectionString = builder.AddConnectionString("cosmosDb");

    management.WithReference(serviceBusConnectionString, "serviceBus")
